...
const generateRefreshandAccessToken = async (userId) => {
  try {
    ...
    // To find the user form db by Id
    const findUser = await User.findById(userId);
 
    // To generate the access and refresh token
    ...
    throw new ApiError(
      500,
      `Something went wrong while generating the refresh and access token: ${error}`
    );
  }
};
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  if (!(username || email)) {
    throw new ApiError(
      400,
      'You have to give one of the following: email or username'
    );
  }
  ...
  let { refreshToken, accessToken } = await generateRefreshandAccessToken(
    user._id
  );
...
const changedPassword = asyncHandler(async (req, res) => {
  ...
  if (!isPasswordValid) {
    throw new ApiError(400, 'Unauthorized request: Invalid old password');
  }
...
const updateAccount = asyncHandler(async (req, res) => {
  ...
  if (!(fullname || email)) {
    throw new ApiError(
      400,
      'All fileds are required to update the account details'
    );
  }
...
const updateUserAvatar = asyncHandler(async (req, res) => {
  ...
  if (!avatar.url) {
    throw new ApiError(
      400,
      'Cover image URL not found while uploading to Cloudinary'
    );
  }
